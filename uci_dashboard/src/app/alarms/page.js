...
// import { getAlarmsData } from '../../webServices/UCIAPIController';
...
//                 </div>
//                 {/* Calendar View */}
//                 <div className="border rounded-lg bg-white p-4 shadow-md">
...
//                     <h2 className="font-bold text-lg">Production Line:</h2>
//                     <select
...
//             {/* Alarms Table */}
//             <div className="rounded overflow-x-auto bg-white shadow-md border border-gray-400">
...
//                                 <th
//                                     key={header.key}
//                                     className="px-4 py-2 text-left text-sm font-bold text-black uppercase tracking-wider"
//                                 >
//                                     {header.label}
//                                 </th>
//                             ))}
...
//                                     <td className="px-4 py-2 text-sm text-gray-900">{formatDateTime(alarm.timestamp)}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{INSTANCE_MAPPING[alarm.instanceid] || alarm.instanceid}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{alarm.alarmtypeid}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{alarm.alarmlabel}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{alarm.isactive ? 'True' : 'False'}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{formatDuration(alarm.duration)}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{formatDateTime(alarm.starttime)}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{formatDateTime(alarm.endtime)}</td>
//                                     <td className="px-4 py-2 text-sm text-gray-900">{alarm.serverity}</td>
//                                 </tr>
//                             ))
//                         )}
//                     </tbody>
...
const INSTANCE_MAPPING = {
    '39ff4795-63f7-4c3c-afe5-90d5133115d7': 'GRA Assembly Alarm',
    '27ab18b6-152b-4382-8801-dd87b4726418': 'GCS Assembly Alarm',
    '2e1cc646-e595-4ccc-9638-c5a14a1c98c0': 'SPC Assembly Alarm',
};

const SUBASSEMBLY_MAPPING = {
    '39ff4795-63f7-4c3c-afe5-90d5133115d7': ['GRA Laser', 'GRA P1', 'GRA P2', 'GRA P3'],
    '27ab18b6-152b-4382-8801-dd87b4726418': ['GCS Laser', 'GCS P1', 'GCS P2', 'GCS P3'],
    '2e1cc646-e595-4ccc-9638-c5a14a1c98c0': ['SPC Laser', 'SPC P1', 'SPC P2', 'SPC P3'],
};
...
const AlarmPage = () => {
    ...
    const generateCalendarData = () => {
        ...
        MONTHS.forEach((month, index) => {
            const daysInMonth = getDaysInMonth(index + 1, currentYear);
            if (index >= currentMonth - 1 && index <= currentMonth) {
                // Real-time data for the last two months
                calendarData[month] = Array(daysInMonth).fill().map(() => ({
    ...
    const hardcodedAlarms = [
        { timestamp: '2025-02-09T14:00:00Z', instanceid: '39ff4795-63f7-4c3c-afe5-90d5133115d7', alarmtypeid: 'AL001', alarmlabel: 'R1 Alarm', isactive: true, duration: 120, starttime: '2025-02-09T12:00:00Z', endtime: '2025-02-09T14:00:00Z', serverity: 'HIGH' },
        { timestamp: '2025-02-08T11:00:00Z', instanceid: '39ff4795-63f7-4c3c-afe5-90d5133115d7', alarmtypeid: 'AL002', alarmlabel: 'R2 Alarm', isactive: false, duration: 90, starttime: '2025-02-08T10:00:00Z', endtime: '2025-02-08T11:00:00Z', serverity: 'MEDIUM' },
        // Add more hardcoded alarms here
    ];

    const totalPages = Math.ceil(hardcodedAlarms.length / ROWS_PER_PAGE);
    const startIndex = (currentPage - 1) * ROWS_PER_PAGE;
    const endIndex = startIndex + ROWS_PER_PAGE;
    const currentAlarms = hardcodedAlarms.slice(startIndex, endIndex);
    ...
    const exportToCSV = () => {
        const headers = tableHeaders.map((header) => header.label).join(',');
        const rows = hardcodedAlarms.map(alarm =>
            tableHeaders.map(header => `"${alarm[header.key] || ''}"`).join(',')
        );
        const csvContent = [headers, ...rows].join('\n');
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
                ...
                <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className={`p-1 rounded transition-colors ${currentYear >= new Date().getFullYear()
                                ? 'bg-gray-300 cursor-not-allowed'
                                : 'bg-blue-500 hover:bg-blue-600 text-white'
                                }`}
                        >
...
Low: ${day.low}`}
                ...
                <div className="flex items-center gap-8">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            setSelectedSubassembly(''); // Reset the subassembly on change
                        }}
                    >
                        {Object.entries(INSTANCE_MAPPING).map(([id, name]) => (
                            <option key={id} value={id}>
                    ...
                    {/* Subassembly Line Dropdown */}
                    {selectedInstance && (
                        <div className="flex items-center gap-8">
                            <h2 className="font-bold text-lg">Subassembly Line:</h2>
                            <select
                                className="w-56 p-3 bg-gray-100 border-b-2 border-blue-400 rounded-md"
                                value={selectedSubassembly}
                                onChange={(e) => setSelectedSubassembly(e.target.value)}
                            >
                                {SUBASSEMBLY_MAPPING[selectedInstance]?.map((subassembly, index) => (
                                    <option key={index} value={subassembly}>
                                        {subassembly}
                                    </option>
                                ))}
                            </select>
            ...
            <div className="rounded overflow-x-auto bg-white shadow-md border border-gray-400">
                <table className="min-w-full divide-y divide-gray-400">
                    ...
                    <tbody className="bg-white divide-y divide-gray-400">
                        {currentAlarms.map((alarm, index) => (
                            <tr key={index} className={`divide-x divide-gray-400 ${alarm.isactive ? 'bg-red-300 hover:bg-red-400' : 'bg-gray-200 hover:bg-gray-300'}`}>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.timestamp}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{INSTANCE_MAPPING[alarm.instanceid] || alarm.instanceid}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.alarmtypeid}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.alarmlabel}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.isactive ? 'True' : 'False'}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.duration}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.starttime}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.endtime}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{alarm.serverity}</td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="flex justify-between items-center mt-4">
                <button
                    onClick={handlePrevPage}
                    disabled={currentPage === 1}
                    className="px-4 py-2 bg-gray-300 rounded-md hover:bg-gray-400 transition-colors disabled:cursor-not-allowed"
                >
                    Previous
                </button>
